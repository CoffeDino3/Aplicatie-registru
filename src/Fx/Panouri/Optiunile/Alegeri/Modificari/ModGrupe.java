...
public class ModGrupe {
    ...
    public ModGrupe(App app) {
        ...
        HBox headerRow=new HBox(0,
                ...
                creeazaHeaderCelula("Grupa",0.18,root),
                creeazaHeaderCelula("An",0.10,root),
                creeazaHeaderCelula("Clasa",0.10,root),
                creeazaHeaderCelula("Nr",0.10,root),
        ...
        Runnable refreshIds=() -> {
            ...
            try {
                for (Grupe g : GrupeCRUD.getAll()) {
                    cbId.getItems().add(String.valueOf(g.getId()));
                }
        ...
        Text tNr=new Text("-");
        Text tSpec=new Text("-");
        HBox dataRow=new HBox(0,new StackPane(cbId),creeazaDataCelula(tGrupa,0.18,root),
                creeazaDataCelula(tAn,0.10,root),creeazaDataCelula(tClasa,0.10,root),
                creeazaDataCelula(tNr,0.10,root),creeazaDataCelula(tSpec,0.21,root));
        dataRow.setAlignment(Pos.CENTER);
        final Grupe[] grupaSelectata={null};
        ...
        HBox checkRow=new HBox(50,creeazaCheckLabel("An",ckAn,root),creeazaCheckLabel("Clasa",ckClasa,root),
                creeazaCheckLabel("Nr",ckNr,root),creeazaCheckLabel("Specialitate",ckSpec,root));
        ...
        for (Specialitati s : Specialitati.values()) {
            cbSpecField.getItems().add(s.getNume());
        }
